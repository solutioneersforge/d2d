...
{
    public class FunctionAppMerchantDetails
    {
        private readonly IMerchantDetailsRepository _merchantDetailsRepository;
        private readonly IValidatedTokenService _validatedTokenService;
        public FunctionAppMerchantDetails(IMerchantDetailsRepository merchantDetailsRepository
            , IValidatedTokenService validatedTokenService)
        {
            _merchantDetailsRepository = merchantDetailsRepository;
            _validatedTokenService = validatedTokenService;
        }

        [FunctionName("FunctionAppMerchantDetails")]
            ...
            ILogger log)
        {
            try
            {
                ...
                var resultData = await _merchantDetailsRepository
                    .GetMerchantDetailsAsync(tokenValidation.userId);
                return new OkObjectResult(new
                {
                    Data = resultData,
                    Message = "Success",
